function [boun_line]=gmsh_write_refinment(Pleft,Pright,Pdown,Pup,P1,P2,P3,P4,Pmain,Pmain2,PB,k1,k2,k3,k4,k5,gmsh_file,lc,lc2,pts)
...
npr=100000; % the start number for refinement
...
for i=1:k5   
    if Pmain2{1,i}==0
        ...
        polyin = polyshape(ptmp); [polycenterx,polycentery]= centroid(polyin); npr=npr+1;fprintf(fid,'%s\n',['Point(',num2str(npr),')={',num2str(polycenterx), ',' , num2str(polycentery) ,',', '0,lc2};']);
        fprintf(fid,'Point{%d} In Surface {%d};\n',npr,lp);
        RK(i)=lp;
    else
        rk=zeros(1,1);irk=0;
        for ii=1:Pmain2{1,i}
